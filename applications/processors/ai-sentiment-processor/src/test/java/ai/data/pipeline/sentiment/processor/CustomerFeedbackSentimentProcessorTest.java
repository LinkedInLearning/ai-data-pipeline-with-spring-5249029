...
class CustomerFeedbackSentimentProcessorTest {
    ...
    @Mock
    private ChatClient.ChatClientRequestSpec userResponse;
    @Mock
    ...
    void positiveSentiment() {
        ...
        when(prompt.user(any(Consumer.class))).thenReturn(userResponse);
        when(userResponse.call()).thenReturn(callResponseSpec);
        when(callResponseSpec.entity(any(Class.class))).thenReturn(expected.sentiment());
